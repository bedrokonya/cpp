...
class ImplicitTreap : public InterfaceImplicitKeyTree{
 ...
 public:
    ...
    int find(Node* v, int key); // находит позицию ноды в массиве, в которой записан ключ key
...
ImplicitTreap::Node* ImplicitTreap::Remove(Node* v, int pos) {
...
ImplicitTreap::Node* ImplicitTreap::Reverse(Node* &v, int a, int b) {
    Node* l;
    Node* m;
    Node* r;
...
ImplicitTreap::Node* ImplicitTreap::findNextElement(Node* v, int key) {
    return nextElement(v, nullptr, key);
}

ImplicitTreap::Node* ImplicitTreap::nextElement(Node* v, Node* successor, int key) {
    Node* tmp = v;
    if (tmp == nullptr)
        return successor;
    while (tmp != nullptr) {
